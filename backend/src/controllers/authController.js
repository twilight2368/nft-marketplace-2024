  ...
  try {
    ...
    if (data.rows[0]) {
      ...
      const insert = await pool.query(
        "insert into users (user_name, password, amount, email) values ($1, $2, $3, $4)",
        [input.username, hashpass, 0, input.email]
      );
      res.json(insert);
    }
  } catch (error) {
  ...
  try {
    ...
    if (something.rows[0]) {
      ...
      if (check_pass) {
        ...
        res.status(200).json({ username: username_d, userid: userid });
      } else {
